...
####Task Summary
1.  Create a cloud connection beteween AWS account and Tenable Cloud Security (Tenable.cs),
...
____
**Documentation Notes:**
- Tenable.cs is used interchangeably with Tenable Cloud Security (Tenable.cs)
- Tenable.io is used interchangeably with Tenable Vulnerability Management
_____

### Prerequisites
- Familiar with Tenable Cloud Security (Tenable.cs) Dashboard 
...
##### Download YAML 
** Permissions are required.**  This file is a cloud formation template used to provision an IAM role in the target AWS cloud account and configuring it for Tenable.cs to read the resources in teh same account.
...
#### Create a cloud connection on Tenable Cloud Security (Tenable.cs)


1.  On Tenable Cloud Security (Tenable.cs), create an AWS connection,
1.  From the Tenable Cloud Security (Tenable.cs) dashboard, goto the the left menu and click on the (+) symbol,
...
To run a cloud scan after onboarding your cloud accounts, you must select and run a scan profile. Tenable.cs provides a default scan profile for each cloud provider. You can also create your custom scan profiles. After creating a scan profile, you can run a Misconfiguation Scan (IaC) and Cloud scan or a Vulnerability Scan (Lab03)
...
1.  Rescan the project (ie **Lab02-project**).
